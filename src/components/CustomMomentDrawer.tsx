...
} from "../lib/customMomentValidation";
...
export function CustomMomentDrawer({
  ...
  const handleAddMedia = async () => {
    ...
    if (!mediaFile) {
      setMediaError(`Choose an ${mediaType} file.`);
      return;
    }
    const error = validateCustomMomentUpload(mediaType, mediaFile);
    if (error) {
      setMediaError(error);
      return;
    }
    ...
    try {
      ...
      const storageError = validateCustomMomentMediaStorage(form.media, dataUrl);
      if (storageError) {
        setMediaError(storageError);
        return;
      }
      const media = createUploadedMomentMedia({
        type: mediaType,
        dataUrl,
      ...
      setMediaError("Could not read that file. Choose another file and retry.");
    } finally {
      setMediaPending(false);
    }
  };
    ...
    >
      <div className="space-y-4">
        ...
        <fieldset className="rounded-md border border-bordermid bg-panel/30 p-3">
          <legend className="px-1 font-mono text-[10px] uppercase tracking-widest text-textmuted">
            Media attachments (optional)
          </legend>
          <p className="mb-3 font-mono text-[11px] text-textmuted">
          ...
          <div className="grid gap-3 sm:grid-cols-[140px_1fr]">
            <label className="block">
              <span className="mb-1.5 block font-mono text-[10px] uppercase tracking-widest text-textmuted">
                Media type
              </span>
              <select
                value={mediaType}
            ...
            {mediaType === "youtube" ? (
              ...
              <label className="block">
                <span className="mb-1.5 block font-mono text-[10px] uppercase tracking-widest text-textmuted">
                  {CUSTOM_MOMENT_MEDIA_TYPE_LABEL[mediaType]} file
                </span>
                <input
                  ref={mediaFileRef}
                  type="file"
        ...
        <label className="flex items-start gap-3 rounded-md border border-bordermid bg-panel/40 px-3 py-2">
          ...
          <span className="text-sm text-text">
            ...
            <span
              ...
              className="mt-1 block font-mono text-[11px] text-textmuted"
            >
        ...
        <div>
          ...
          <p
            id={visibilityHelpId}
            className="mt-1.5 font-mono text-[11px] text-textmuted"
          >

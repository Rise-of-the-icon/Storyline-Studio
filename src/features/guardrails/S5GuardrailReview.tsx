...
import { Card, type CardState } from "@/shared/ui/Card";
import { EditorialReviewModal } from "@/features/guardrails/EditorialReviewModal";
...
function flagCardState(
  ...
  if (review.status === "Rejected") return "rejected";
  if (review.status === "Deferred") return "deferred";
  if (review.status === "NeedsReview" && isHigh) return "flagged";
  return "needs-review";
}
...
function FlagRow({
  ...
  return (
    <Card
      as="li"
      state={flagCardState(review, isHigh)}
    ...
    >
      ...
      {review.status === "Reviewed" && review.editorialNote && (
        <p className="mt-3 rounded-md border border-bordermid bg-panel/40 px-3 py-2 font-body text-xs text-textsub">
...
function SummaryCard({
  ...
  return (
    ...
    >
      ...
      {autoCleared > 0 && (
        <p className="mt-3 meta-mono text-textmuted">
  ...
  return (
    <div className="mx-auto max-w-[680px] px-4 pb-action-bar pt-6">
      ...
      {reviews.length === 0 ? (
        <div
          role="status"
          className="mt-8 flex flex-col items-center gap-3 rounded-lg border border-ok/40 bg-okfaint px-6 py-8 text-center"
        >
      ...
      {hasRejections && (
        <div className="mt-8 rounded-lg border border-danger/40 bg-dangerfaint px-4 py-3">
      ...
      <WizardActionBar
        ...
        primary={{
          ...
          describedBy: "s5-save-helper",
        }}
        helper={footerStatus}
        helperMobile={footerStatus}
      />

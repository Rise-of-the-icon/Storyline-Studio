...
import { TimelineEventSkeleton } from "@/shared/ui/Skeleton";
import { TimelineRevealItem } from "@/shared/ui/TimelineRevealItem";
...
function EventRow({ event, onApprove, onDefer }: EventRowProps) {
  ...
  return (
    <Card
      as="article"
      state={approved ? "reviewed" : "default"}
    ...
    >
      ...
      >
        <Card.Title id={`evt-${event.id}-title`}>{event.title}</Card.Title>
        <Card.Meta>
          {event.year}
      ...
      {hasSourceNotes && (
        <Disclosure title="Source notes" className="mt-3">
          <div className="space-y-2 meta-mono">
  ...
  return (
    ...
    >
      ...
      {isHeuristic && (
        <Callout
          tone="warning"
          eyebrow="Heuristic timeline"
          role="note"
          className="mt-6"
        >
          Events were auto-extracted from the Wikipedia summary — confidence is
          approximate. Review every event before approving; expand &ldquo;Source
          notes&rdquo; to verify the citation.
        </Callout>
      )}
      ...
      <WizardActionBar
        ...
        primary={{
          ...
          describedBy: "s3-continue-helper",
        }}
        helper={
          <>
